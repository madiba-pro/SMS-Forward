...
public class PhoneNumberMatcherTest {
    ...
    @Test
    public void fallsBackToCaseInsensitiveMatchForAlphanumericSenderIds() {
        // Alphanumeric sender IDs (e.g. bank/OTP senders) aren't parseable phone numbers,
        // so both sides fail to normalize and we fall back to comparing the text itself.
        PhoneNumberMatcher.Normalizer failedNormalizer = number -> null;

        assertTrue(PhoneNumberMatcher.areSame("AMAZON", "amazon", failedNormalizer));
        assertTrue(PhoneNumberMatcher.areSame(" HDFC-BK ", "hdfc-bk", failedNormalizer));
        assertFalse(PhoneNumberMatcher.areSame("AMAZON", "AMAZOM", failedNormalizer));
    }
