...
public class SMSReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        final boolean includeDestination = sharedPreferences.getBoolean(
                context.getString(R.string.key_include_destination), true);

        final String senderFilterMode = sharedPreferences.getString(
                context.getString(R.string.key_sender_filter_mode), SenderFilter.MODE_OFF);
        final String senderFilterNumbers = sharedPreferences.getString(
                context.getString(R.string.key_sender_filter_numbers), "");
        ...
        // Which of the device's own lines (SIM/subscription) received this broadcast, if
        // determinable. Same for every message in this intent, so resolve it once.
        final String destinationLabel = includeDestination ? SimUtils.getDestinationLabel(context, intent) : null;
        ...
        for (Object messageObj : pduObjects) {
            ...
            if (areSamePhoneNumber(senderNumber, targetNumber, context)) {
                ...
                if (!SenderFilter.shouldForward(senderNumber, senderFilterMode, senderFilterNumbers,
                        getNetworkCountryIsoOrDefault(context)))
                    continue;

                if (enableSMS && !targetNumber.equals(""))
                    Forwarder.forwardViaSMS(senderLabel, rawMessageContent, targetNumber, keepSmsTogether, destinationLabel);
                if (enableTelegram && !targetTelegram.equals("") && !telegramToken.equals(""))
                    Forwarder.forwardViaTelegram(senderLabel, rawMessageContent, targetTelegram, telegramToken, destinationLabel);
                if (enableWeb && !targetWeb.equals(""))
                    Forwarder.forwardViaWeb(senderLabel, rawMessageContent, targetWeb, destinationLabel);
            }
        }
    }
